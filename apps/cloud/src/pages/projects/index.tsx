...
const Projects: NextPage = () => {
    ...
    const [createProjectState, setCreateProjectSate] = useState<{ valid: boolean, errorMessage?: string }>({ valid: true });
    ...
    const submitProjectName = () => {
        createProject.mutate({ name: inputBindings.value }, {
            onSuccess: closeModal,
            onError: (error) => {
                setCreateProjectSate({ valid: false, errorMessage: error.data?.code === "CONFLICT" ? error.message : "Invalid project name" })
            },
        })
    }
    ...
    return (
        ...
        <Page className="page-height" dotBackdrop={true}>
            <Page.Header>
                <div className="flex items-center gap-3">
                    <Text h2>My Projects</Text> <Button auto icon={<Plus />} px={0.6} type="success" onClick={() => setVisible(true)} />
                </div>
            </Page.Header>
            <Page.Content>
        ...
        <Modal {...modalBindings} onClose={closeModal}>
            ...
            <Modal.Content>
                <Input placeholder="Enter Project name" type={createProjectState.valid ? "default" : "error"} value={inputBindings.value} onChange={inputChange} width="100%" />
                { createProjectState.valid ? <></> : <><Spacer /> <Text span type="error">{createProjectState.errorMessage}</Text></>}
            </Modal.Content>
            <Modal.Action passive onClick={closeModal}>Cancel</Modal.Action>
            <Modal.Action passive onClick={submitProjectName}>Create</Modal.Action>
        </Modal>
        </>
    )
}
